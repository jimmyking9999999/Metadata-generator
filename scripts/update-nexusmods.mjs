...
const DISCORD_NOTIFICATION_BLACKLIST = new Set(["Casualities After Dark"]);
...
async function main() {
  ...
  const skipLargeArchives = process.argv.includes("--skip-large-archives");
...
async function runFullSync({ apiKey, appVersion, gameDomains, entryByKey, skipLargeArchives }) {
  for (const gameDomain of gameDomains) {
...
function buildNotification(previousEntry, nextEntry, archiveContext) {
  if (!nextEntry?.Links?.NexusMods || DISCORD_NOTIFICATION_BLACKLIST.has(nextEntry.Name)) {
    return null;
  }
...
async function getModChangelogs({ apiKey, appVersion, gameDomain, modId }) {
  try {
    const response = await nexusRest(`/games/${encodeURIComponent(gameDomain)}/mods/${modId}/changelogs.json`, apiKey, appVersion);
    return normalizeChangelogs(response);
  } catch (error) {
    logWarn("CHANGELOG", `Could not load changelogs for mod ${modId}. ${error.message}`);
    return null;
  }
}

function normalizeChangelogs(response) {
  const source = response?.changelogs ?? response;
  const entries = Array.isArray(source)
    ? source
    : Object.entries(source ?? {}).map(([version, changelog]) => ({ version, changelog }));
  return entries
    .map((entry) => {
...
function decodeHtmlEntities(value) {
  return String(value ?? "").replace(/&(#(?:x[\da-f]+|\d+)|amp|lt|gt|quot|apos|#39);/gi, (entity, code) => {
...
function getFileSizeBytes(file) {
  for (const [field, multiplier] of [["size_in_bytes", 1], ["size_bytes", 1], ["file_size", 1], ["size", 1], ["size_kb", 1024], ["size_mb", 1024 * 1024]]) {
